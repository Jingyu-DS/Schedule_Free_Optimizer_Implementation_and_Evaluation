...
for epoch in range(n_epochs):
    ...
    for X_batch, y_batch in train_loader:
        ...
        # Update Polyak average after each step
        steps += 1
        with torch.no_grad():
...
# After training, create a Polyak averaged model for evaluation
polyak_model = MyModel(num_classes=num_classes).to(device)
with torch.no_grad():
    for name, p in polyak_model.named_parameters():
        p.copy_(avg_params[name])

# Evaluate the Polyak averaged model
polyak_model.eval()
correct = 0
total = 0
with torch.no_grad():
    for X_batch, y_batch in test_loader:
        X_batch, y_batch = X_batch.to(device), y_batch.to(device)
        outputs = polyak_model(X_batch)
        _, predicted = torch.max(outputs, 1)
        correct += (predicted == y_batch).sum().item()
        total += y_batch.size(0)

test_acc = correct / total
print(f"Polyak Averaged Model Test Accuracy: {test_acc:.4f}")
